const products = JSON.parse(dataProductsInCart)
...
nameContainer.textContent = 'Cart Items'

productsBox.before(nameContainer)
...
products.forEach(product => {
    ...
    productPrpQuantity.innerHTML = `<label for="quantity">Quantity:</label>
            <input class="cart-product-dsc__quantity" type="number" step="1" min="1" max="99" id="quantity" value="2" />`
...
closeEls.forEach(element => {
    let elemCount = productsBox.children.length
    element.addEventListener('click', function (e) {
        clickCount++
        element.parentElement.remove()
        if (clickCount === elemCount) {
            cartItemsEl.remove()
        }
    })
})
